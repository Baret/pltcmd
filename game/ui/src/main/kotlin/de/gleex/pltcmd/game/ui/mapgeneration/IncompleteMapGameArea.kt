 ...
 */
class IncompleteMapGameArea(size: Size, worldWidthInTiles: Int, worldHeightInTiles: Int) :
        BaseGameArea<Tile, IncompleteMapBlock>(
                ...
                initialContents = size.fetchPositions()
                        ...
                        .associateWith {
                            val xNeeded = (size.width.toDouble() / (worldWidthInTiles / MainCoordinate.TILE_COUNT)).roundToInt()
                            val yNeeded = (size.height.toDouble() / (worldHeightInTiles / MainCoordinate.TILE_COUNT)).roundToInt()
